...
This manager is called by the **WeatherDataFragment**, the **UpdateWidgetService**, the **GraphDataFragment**, the **PhotoManager**, the **WeatherDataActivity**.
###### InternetManager
...
##### Models
###### HttpRequestModel
This model contains information about the type of data request to be done. With the overloads of the constructor, the fields of this model are set in a way so with each constructor the rest of the code can reconstruct what kind of request it is. It also contains the specific url. This is why this model is passed on during the request process.
...
The challenge was especially in getting the image resized with the width of the screen as a maximum but conserving the aspect ratio. Most ways need a width and height specified. Then to get this system working from the context of the app and the widget was some figuring out. In total this implementation cost me a day which does not seem like much but I had planned to do much more in this day. This affected in how far I have come implementing my application till now.
#### Changes
...
There are no major changes in the code itself that I would like to make. I still have a lot of inspiration for little UI changes though. Mostly in the layout but also a few in the UI itself. For example given more time I would have liked to change all dialogs to alert dialog interfaces to give the app a more unified feel in design. I still have inspiration to add animations in a few places. And work on the relation between the photo in the WeatherDataFragment and the weather data itself, by making the data able to slide from under the photo up over the photo when the user taps on the data.